...
Agent Run은 **한 번의 실행 시도**다. Job은 **목표와 상태를 소유하는 지속 단위**다.
이 둘을 분리했기 때문에 Run이 죽어도, 리뷰에서 반려돼도, 모델을 바꿔도 Job은 이어진다.

```text
Job ──┬── Agent Run #1 (구현)   → Session A → Tool Calls
...
이 표는 `JobState::can_transition_to()` **한 곳**에만 존재한다. 스케줄러도 workflow
plugin도 새 전이를 발명할 수 없다.

### 강제되는가

`Job::state`와 `depends_on`은 **private**이다. 공개 필드였다면 이렇게 무력화된다.

```rust
job.state = JobState::Completed;   // 컴파일되지 않는다
```

유일한 진입점은 검증하는 두 함수다.

```rust
graph.apply(id, JobState::Completed)?;   // RUNNING 이면 Err
job.transition_to(JobState::Running)?;  // 예산 소진이면 Err
```

`depends_on`도 같은 이유로 private이다 — 직접 수정하면 `JobGraph::insert()`의 순환
검사를 우회할 수 있다.

**역직렬화도 같은 문을 통과한다.** 필드를 private으로 만들어도 상태 파일이
`state: "COMPLETED"`나 순환 의존을 주입할 수 있으면 아무것도 얻지 못한다. `JobGraph`의
`Deserialize`는 `from_jobs()`를 거치며 디스크에서 온 그래프도 메모리에서 만든 것과 똑같이
검증한다.

### 세 가지 결정

**종료 상태는 흡수 상태다.**
`COMPLETED`에서 나가는 경로는 없다. 재개는 새 Job을 만든다는 뜻이며, 그래야 이력이
정직하다. "완료됐다가 다시 실행 중"인 Job은 감사할 수 없다.

**`RUNNING → COMPLETED` 직행은 없다.**
반드시 `REVIEW`를 거친다. 건너뛸 수 있는 게이트는 게이트가 아니다.
리뷰가 필요 없는 워크플로는 `Job::complete_without_review()`를 쓰는데, 이 함수도
`REVIEW`를 **통과**한다. 로그에 "열린 게이트"가 남지 "없는 게이트"가 남지 않는다.

**`WAITING`은 종료가 아니다.**
`is_settled()`는 `WAITING`을 살아있는 상태로 센다. approval-gate 워크플로가 사람 승인을
기다리며 파킹한 순간을 "끝남"으로 보면, Job Runtime이 매번 사람보다 먼저 종료된다.
...
그래서 **`READY → FAILED` 전이가 필요하다.** 이것이 없으면 예산이 소진된 Job은 `READY`에
...
```rust
graph.newly_ready()   // 의존성이 모두 COMPLETED 인 PENDING job
graph.blocked()       // 의존성 중 FAILED/CANCELLED 가 있는 비종료 job
graph.is_settled()    // 더 이상 진행 불가
```

**실패한 의존성은 후속 Job을 만족시키지 않는다.** 후속은 영원히 `PENDING`으로 남는다 —
...
pub trait Workflow: Send + Sync + Debug {
    fn name(&self) -> &str;
    async fn next(&self, graph: &JobGraph) -> Result<Vec<JobId>>;
    async fn requires_review(&self, graph: &JobGraph, job: &Job) -> bool;
}
```

Workflow는 그래프 모양에 대한 **순수 정책**이며 그래프를 **변경하지 않는다.**
상태 전이는 Job Runtime이 적용한다. 버그 있는 workflow plugin은 진행을 멈출 수는 있어도
상태를 오염시킬 수는 없다.

`next()`가 빈 벡터를 반환하는 것은 "지금은 없음"이지 "끝"이 아니다.
완료 판정은 `JobGraph::is_settled()`가 한다.
...
| `review-gate` | 모든 Job이 `REVIEW`를 거침 |
| `approval-gate` | 지정된 Job에 사람 승인 요구 |
...
pub trait Scheduler: Send + Sync + Debug {
    ...
    async fn claim(&self, job_id: JobId) -> Result<bool>;
    async fn release(&self, job_id: JobId) -> Result<()>;
}
```
...
리뷰는 `Job.acceptance`의 체크 가능한 조건에 대해 판정한다.
...
- **워크스페이스 동시성** — 한 워크스페이스에서 두 Job이 동시에 실행되면 파일이
  충돌한다. git worktree 분리? 순차 강제? Phase 5 착수 전 결정 필요.
  (`architecture.md` 열린 질문 §11-5)
- **부분 실패 그래프의 재개** — `blocked()` Job들을 어떻게 되살릴 것인가.
  의존성을 고친 뒤 수동 `retry` 명령? 자동?
- **Job 영속화** — 세션과 같은 event-sourced 방식? 별도 상태 저장?
  현재 계약은 둘 다 허용한다. 어느 쪽이든 `ReviewVerdict`는 durable해야 한다 — 세션 승인과
  같은 이유로, "누가 이 Job을 승인했는가"는 감사 질문이다 (Phase 5.9).
